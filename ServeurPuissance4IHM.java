			...
			while ((line = in.readLine()) != null) {
				if (line.equals("QUIT")) {
					...
					try {
						...
						if (playerRoles.get(client) == currentJoueur) {
							if (playMove(col, currentJoueur)) {
								if (checkVictory(currentJoueur)) {
									broadcastGrid();
									broadcast("VICTORY:" + currentJoueur);
									broadcast("CHAT:SYSTEM:Le joueur " + currentJoueur + " a gagné !");
									resetGame();
								} else {
									currentJoueur = (currentJoueur == 1) ? 2 : 1;
									broadcastGrid();
									broadcast("TURN:" + currentJoueur);
								}
							}
						}
